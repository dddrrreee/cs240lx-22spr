...
void simple_boot(int fd, const uint8_t *buf, unsigned n) { 
    ...
    trace_put32(fd, n);
    uint32_t coll_res_cksum = crc32(buf, n);
    trace_put32(fd, coll_res_cksum);
    ...
    if (op != GET_CODE){
        panic("Not received GET_CODE\n");
    }

    if (get_op(fd) != coll_res_cksum){
        panic("Cksum does not match\n");
    }

    // 4. send a PUT_CODE + the code.
